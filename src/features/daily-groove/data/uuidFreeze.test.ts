...
// without a red test — which is the point. The manifest and the generator's
// catalogue.json move together on one render, so pinning them against each
...
//
// The same table under the same name lives on the generator side, in the
// uuidFreeze.test.ts beside catalogue.json, which this file may not import from
// — `grep -rn FROZEN_UUIDS` finds both halves. Both are pinned against the
// manifest's bindings, so neither can drift from the other.
...
describe('the uuids a share link resolves are frozen', () => {
  ...
  it('pins a uuid for every groove the manifest ships', () => {
    ...
    const unpinned = GROOVES.filter((groove) => !frozen.has(groove.id))
    const paste = unpinned
      .map((groove) => `  ['${groove.id}', '${groove.uuid}'],`)
      .join('\n')
    expect(
      unpinned.map((groove) => groove.id),
      unpinned.length === 0
        ? ''
        : 'newly minted grooves are not frozen yet — paste these lines into ' +
          `FROZEN_UUIDS in this file and in the generator-side half:\n${paste}`,
    ).toEqual([])
  })

  it.each(FROZEN_UUIDS)('resolves %s to %s', (id, uuid) => {
    const groove = GROOVES.find((entry) => entry.id === id)
    expect(groove, `${id} has left the manifest — a frozen share link now resolves nothing`)
      .toBeDefined()
    expect(groove!.uuid, `${id}'s uuid moved — every link ever shared for it is dead`).toBe(uuid)
  })
})
