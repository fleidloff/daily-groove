...
Q1-A, Q2-B and Q3-A, as ticked. One decision was taken in chat during the build and is recorded here rather than as a late question: **the page's labels are plain literals in `GroovePreview.tsx`.** `docs/coding-guidelines.md`'s "no user-facing string is written inside a component" covers feature components and routes, but the rule exists so that a second language replaces `src/lib/snippets/en/` in one place — and a page `pageExtensions` keeps out of the production build has no second language to reach. The waiver stops at `components/dev/`; `eslint.config.mjs` gained no ignore entry to buy it.

* `src/app/dev/grooves/page.dev.tsx` — new. A thin server component: `PageShell` → `Container` → `main` → `<GroovePreview />`, reached through `@/features/daily-groove` because the eslint zone binds `src/app` to the slice's `index.ts`. It takes no props — `PageProps<'/dev/grooves'>` does not exist in a build where this is not a route. It is the only source file in the repo that no build compiles; `tsc` still covers it.
* `next.config.ts` — exports `pageExtensionsFor(env)`: the base list `['tsx','ts','jsx','js']`, plus `'dev.tsx'` when the env is `development`. `pageExtensions` calls it with `process.env.NODE_ENV`. The list is additive, so every existing `page.tsx` resolves exactly as before.
* `src/features/daily-groove/components/dev/GroovePreview.tsx` — new. Builds the schedule with `nextDayStart` and `selectGrooveForDate`, one row per day from today, stopping once every uuid in `GROOVES` has appeared and capped at `2 * GROOVES.length`. Each row is a `Chip` — which is where `aria-pressed` comes from — reading `iso · name · scale · chord · progression · N bpm`. The picked row renders a player **keyed by its ISO date**, so `useTransport`'s `useState` initialiser re-captures the source instead of staying on the first groove picked. That player holds a `PlayControl` and one lick button per `LICK_VARIATIONS`, each calling `useModeLick` with the transport's `clock` and its own variation.
* `src/features/daily-groove/index.ts` — exports `GroovePreview`. The slice's public surface is now two components.
* `src/features/daily-groove/hooks/useModeLick.ts` — `UseModeLickInput` gains `variation?: number`; the memo returns it when defined and falls back to `variationFor(seed)` otherwise. `GroovePuzzle.tsx` is untouched and keeps the day's seeded pick.
* tests: `components/dev/GroovePreview.test.tsx` (new, 8 cases over D2–D4) and `src/app/dev-route.test.ts` (new, 4 cases over D1). Guards taught the new shape: `structure.test.ts` (`REGIONS` gains `dev`), `route-boundary.test.ts` (`ROUTE_FILES` gains the route), `index.test.ts` (both exact export lists gain `GroovePreview`), `hooks/useModeLick.test.ts` (the explicit variation, and the seed still deciding when none is handed in).
* **Eleven files against the seven `## Notes` predicted.** The four the analysis missed are all tests or guards: `index.test.ts` pins the public surface as an exact list and had to learn the new export; `dev-route.test.ts` is where D1 became checkable at all. No extra module, no extra concept — the size test still passes.
* The red step was checked for vacuousness before any code existed: with a throwaway implementation standing in, sharing one player instead of remounting per row fails D3's second-groove case, dropping the `clock` fails D4's beat assertion, and pinning every lick to variation 0 fails two cases.
* Coverage of the Q3-A loop was re-measured against the shipped manifest over 730 start dates: worst case 58 rows against the cap of 60.
* checks: lint — clean / tsc — clean / test — 142 files, 2868 pass / test:gen — 1065 pass / build — pass, and the build's route list is `/`, `/_not-found`, `/groove/[uuid]`, with no `/dev/grooves`.
* verifier: **pass with gaps** — D2, D3, D4 **done**; D1 **partly**. Its build half is proven by the route list above; its `next dev` half rests on structural evidence only (`page.dev.tsx` exists, `pageExtensionsFor('development')` includes `dev.tsx`), because no test starts a dev server. **Settled by hand:** the page was opened under `npm run dev` and checked, so D1 holds in full.
* Two documents had drifted the moment the fifth folder landed, and were corrected in the same run: `docs/architecture.md`'s shell row said four regions and two routes, and `docs/coding-guidelines.md`'s region rule defines a region as "a subtree of one composer" — which `dev/` is not. The guidelines now say so, and carry the snippets waiver above.
