...
* every user-facing string moves to one central place — today they are written inline wherever they are rendered, so nobody can see what the app actually says
* the point is being able to read the whole voice of the app in one file and change a line without hunting for it
* changing a snippet must not break a test. no test asserts a literal string — it imports the snippet and asserts against that
* which makes the test rule the load-bearing half: a centralised snippet nobody reads from is just a second place the string lives
* this is the centralisation half of feature-B and nothing more — no translation, no locales, no build-time translation step. feature-B keeps those
