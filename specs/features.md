...
| [9](feature-9/) | Natural feel | 🔨 In progress | Makes the grooves sound like a band rather than a sequencer — a real kit, bass and keyboard in place of the cajon and the FM piano, loops whose every pass is a different take, timing and dynamics with limbs, voicings and bass lines instead of arpeggios, a fill to end the last pass, and two more feels carrying four more modes. |
| [10](feature-10/) | Hear the root | 🛠 Ready to implement | Gives the root row a voice — tap a root to hear it against the running groove, so the home note can be found by ear instead of guessed. |
...
progress, 7–10 are planned, and A–B are briefed. These are the gaps that remain
beyond all of them.
